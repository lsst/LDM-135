...
\item
  ...
  \item
    ...
    ``ad hoc'' access for complex queries.
  \item
    Throughput of 1 query per hour with average latency of 8 hours for
...
project requirements (see \citeds{LDM-555}) as well as cost, availability and
...
at which the re-association occurred.
...
solar system object, as well as a re-association time, are included.
Re-association will set the solar system object ID and re-association
...
no off-the-shelf solution meets the above requirements today, an RDBMS
seems a much better fit than a Map/Reduce-based system, primarily due to
...
DBMS relies on \emph{basic} DBMS functionality only, and avoids
...
An prototype implementation of the baseline architecture described above was
developed during the RD phase of LSST, and feasibility was demonstrated
in early testing (\citeds{DMTR-21}, \citeds{DMTR-12}). Productization was
subsequently planned and resourced for the construction phase of LSST and is
presently underway.  The system as currently implemented is described here.
...
engine on each czar to perform lookups on the database's key column
...
instances and will be served via a single fault-tolerant MySQL instance
...
(e.g. at least 1 per minute), the lost time is insignificant. To avoid
...
XRootD. The XRootD
system manages cluster membership, node registration/deregistration,
...
run preset queries on preset data sets both through plain MySQL and
...
A metadata system, as described above, for both static and dynamic metadata is
in place, and basic query management facilities (query cancellation and
monitoring) have been implemented.

The system is currently containerized (Docker) for cluster deployment, and
includes a comprehensive set of unit and regression tests that are integrated
with the build and CI systems.

In addition to background ongoing effort to improve query performance and
extend query coverage, future implementation work includes:
...
\textbf{Automatic data distribution and replication}. An important element of
Qserv's fault tolerance and recovery strategy involves maintaining distributed
replicas of data within the cluster so data remains online in the face of a
reasonable number of concurrent disk or node failures.  We have experimented
successfully with static replication of data at ingest time, but for
production a system which will automatically place data and respond
dynamically to node arrivals and departures (both planned and unplanned) will
be required.  Such a system is currently under development.

\textbf{Improved query management and monitoring tools}. When performing
interactive exploration of an observational data set, users frequently issue
...
additional resources.  A facility to enable clients to temporarily disconnect
from long-running queries and later rendezvous to check query progress and/or
collect results is also required.
...
improvements to our administration scripts.
...
\item
  ...
  If it were to become necessary to support row updates and/or transactions
  directly within Qserv, some considerable changes to the Qserv architecture
...
state-of-the-art in petascale data management and analysis, we continue
