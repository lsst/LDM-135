...
\setDocChangeRecord{%
  ...
  \addtohist{}{2017-07-05}{Move historical investigations to separate documents.}{T.~Jenness}
}
...
requirements.
\secref{design-trade-offs} discusses design trade-offs
...
\secref{other-demonstrations} describes some Qserv demonstrations.
...
\secref{design-trade-offs}). The architecture is periodically revisited: we
...
This section covers the test we run to determine the optimal
...
performance we may expect there for DC3b. These tests are documented in
\citep{spatial-join}. We found
...
\citedsp{LDM-141}. Should we need to, we can partition the largest
...
described in details in \citeds{Document-11701}. We found that
...
described in detail in \citeds{DMTR-12}, \citeds{DMTR-21} and \citeds{LDM-552}.
...
described in details in \citeds{Document-1386}. We found that as long
