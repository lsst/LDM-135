...
\setDocChangeRecord{%
  ...
  \addtohist{4.0}{2017-07-07}{Bring up to date with current status, condense requirements section, re-order sections for improved readability. Approved in \href{https://jira.lsstcorp.org/browse/RFC-358}{RFC-358}.}{Fritz~Mueller}
}
...
express through SQL language; e.g. time-series analyses. For these types of
analyses, we wish to allow users to execute their algorithms in a procedural
language, such as Python, on hardware resources co-located with the production
database data.

One approach being investigated is to allow users to run pre-filter queries on
the production database, with the results streamed to the user processing cluster
where their custom code may run without directly endangering the production database.
This would allow incurred database I/O needs to be satisfied using the database
system's shared scanning infrastructure.

Another approach being investigated would involve keeping one set of replicas of
the catalog data files external to the database system, in a storage format
optimized for an off-the-shelf MR processing framework.  This framework would then
be deployed and run on the user processing cluster independent of the database
system.  The data replication system would then be responsible for transforming
files between database and MR framework formats during data ingest and/or recovery.
